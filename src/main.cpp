...
void opcontrol()
{
	...
	pros::Motor intake(-4);					// Creates a motor on port 7
	...
	while (true)
	{
		...
		{
			leftArcade = !leftArcade; // Toggles control scheme
			pros::delay(200);		  // Waits 200 ms to prevent accidental toggles
		}
		...
		left_mg.move(dir + 0.8 * turn);	 // Sets left motor voltage
		right_mg.move(dir - 0.8 * turn); // Sets right motor voltage
